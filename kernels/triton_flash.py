...
def attention_kernel(
    ...
    # Scale Q once (FlashAttention style)
    scale = tl.math.rsqrt(tl.full([], BLOCK_D, tl.float32))
    q = q * scale
    ...
    for start_n in range(0, SEQ_LEN, BLOCK_N):
        ...
        k = tl.load(k_ptrs, mask=offs_n_curr[None, :] < SEQ_LEN, other=0.0).to(tl.float32)
        ...
        exp_i = tl.exp(m_i - m_new)
        exp_ij = tl.exp(scores - m_new[:, None])

        l_new = l_i * exp_i + tl.sum(exp_ij, axis=1)
